...
export async function loadUsers(context, ids) {
  ...
  for (let id of ids) {
    ...
    let cacheResults = await context.redis.get(id);
    if (cacheResults) {
      cacheResults = JSON.parse(cacheResults);
      users.push({ ...cacheResults });
    } else {
      const results = await context.db.get(id);
      context.redis.set(id, JSON.stringify(results), "EX", 60 * 60 * 24);
      users.push(results ?? null);
    }
  }
  return users;
}
